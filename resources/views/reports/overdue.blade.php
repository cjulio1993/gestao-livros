...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-6">
    <div class="space-y-6">
        <x-dashboard-card
            title="Relatório: Empréstimos Atrasados"
            color="red"
            ...
            description="Lista dos empréstimos que estão com devolução atrasada. Tome as ações necessárias.">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                <div class="flex items-center w-full sm:w-1/2">
                    <div class="relative w-full">
                        ...
                        <input id="overdue-search" type="text" placeholder="Pesquisar por usuário, livro..." class="pl-10 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500" />
                    </div>
                </div>

                <div class="flex items-center space-x-2">
                    <x-button href="{{ route('reports.overdue') }}" color="secondary" outlined>
                        Atualizar
                    </x-button>
            ...
            @if($loans->isEmpty())
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-200">Nenhum empréstimo atrasado</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Ótimo — todos os livros foram devolvidos dentro do prazo.</p>
                </div>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Dias de Atraso</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($loans as $loan)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="h-8 w-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                                                <span class="text-sm font-medium">{{ optional($loan->leitor)->nome ? substr(optional($loan->leitor)->nome, 0, 1) : '-' }}</span>
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $loan->due_date?->format('d/m/Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-red-700 dark:text-red-300 font-semibold">{{ $loan->getDaysOverdue() }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
